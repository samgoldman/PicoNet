...
from packet_manager import PACKET_TYPE_DATA, get_packet_manager
        ...
        if self.blink:
            if self.value: # LED is on
                if time.monotonic_ns() - self.blink_last_change > self.blink_on:
                    get_packet_manager().queue_outgoing_packet(Packet(PACKET_TYPE_DATA, None, 0, 0, 0, TYPE_LED, self.device_id, b'\x2A\x00'))
                    self.value = False
                    self.blink_last_change = time.monotonic_ns()
            elif not self.value: # LED is off
                if time.monotonic_ns() - self.blink_last_change > self.blink_off:
                    get_packet_manager().queue_outgoing_packet(Packet(PACKET_TYPE_DATA, None, 0, 0, 0, TYPE_LED, self.device_id, b'\x2A\x01'))
                    self.value = True
                    self.blink_last_change = time.monotonic_ns()
